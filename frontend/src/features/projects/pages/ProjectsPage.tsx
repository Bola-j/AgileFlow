...
export function ProjectsPage() {
  ...
  return (
    <>
      <PageHeader title="Projects" description="Plan and track delivery in this workspace." actions={<Button onClick={() => setModalOpen(true)}><Plus className="h-4 w-4" />Create project</Button>} />
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {projects.map((project) => <Card key={project.id}><CardHeader><CardTitle>{project.name}</CardTitle><CardDescription>{project.description || "No description"}</CardDescription></CardHeader><CardContent><div className="mb-4 flex flex-wrap gap-2"><Badge value={project.status} /><Badge value={`${formatDate(project.startDate)} - ${formatDate(project.endDate)}`} /></div><div className="flex justify-between gap-2"><Button asChild variant="outline"><Link to={routes.project(project.id)}>Open</Link></Button><Button size="icon" variant="ghost" onClick={() => setDeleteId(project.id)}><Trash2 className="h-4 w-4" /></Button></div></CardContent></Card>)}
      </div>
