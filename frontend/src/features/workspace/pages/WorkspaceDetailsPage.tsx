...
const memberSchema = z.object({ userId: z.string().min(1), role: z.number() });
...
export function WorkspaceDetailsPage() {
  ...
  const memberForm = useForm<z.infer<typeof memberSchema>>({ resolver: zodResolver(memberSchema), defaultValues: { userId: "", role: UserRole.Developer } });
  ...
  const addMember = useMutation({ mutationFn: (values: z.infer<typeof memberSchema>) => workspaceApi.addMember(workspaceId, { userId: values.userId, role: values.role as 0 | 1 | 2 }), onSuccess: async () => { await invalidate(); setMemberOpen(false); memberForm.reset(); toast.success("Member added."); }, onError: (error) => toast.error(getErrorMessage(error)) });
  ...
  const removeMember = useMutation({ mutationFn: (userId: string) => workspaceApi.removeMember(workspaceId, userId), onSuccess: async () => { await invalidate(); setRemoveUserId(null); toast.success("Member removed."); }, onError: (error) => toast.error(getErrorMessage(error)) });
  ...
  return (
    <>
                ...
                <Table><TableHeader><TableRow><TableHead>Name</TableHead><TableHead>Email</TableHead><TableHead>Role</TableHead><TableHead /></TableRow></TableHeader><tbody>
                  {members.map((member) => (
                    <TableRow key={member.userId}>
                      ...
                      <TableCell className="text-right"><Button size="icon" variant="ghost" onClick={() => { setProfileUserId(member.userId); profileForm.reset({ firstName: member.fullName.split(" ")[0] ?? "", lastName: member.fullName.split(" ").slice(1).join(" "), phoneNumber: "", githubUsername: "" }); }}><Pencil className="h-4 w-4" /></Button><Button size="icon" variant="ghost" onClick={() => setRemoveUserId(member.userId)}><UserMinus className="h-4 w-4" /></Button></TableCell>
                    </TableRow>
                  ))}
                </tbody></Table>
              </div>
      ...
      <Modal open={memberOpen} onOpenChange={setMemberOpen} title="Add member" description="Enter the user id returned by the backend identity system."><form className="grid gap-4" onSubmit={memberForm.handleSubmit((values) => addMember.mutate(values))}><Field label="User ID" error={memberForm.formState.errors.userId?.message}><Input {...memberForm.register("userId")} /></Field><Field label="Role"><SelectInput {...memberForm.register("role", { valueAsNumber: true })}><option value={UserRole.Developer}>Developer</option><option value={UserRole.TeamLead}>Team Lead</option><option value={UserRole.Admin}>Admin</option></SelectInput></Field><Button disabled={addMember.isPending}>{addMember.isPending ? "Adding..." : "Add member"}</Button></form></Modal>
      ...
      <ConfirmDialog open={removeUserId !== null} onOpenChange={(open) => !open && setRemoveUserId(null)} title="Remove member" description="This removes the member from the workspace if authorized." busy={removeMember.isPending} onConfirm={() => removeUserId && removeMember.mutate(removeUserId)} />
    </>
  );
}
