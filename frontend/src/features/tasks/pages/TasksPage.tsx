...
export function TasksPage() {
  ...
  return (
    <>
      ...
      <TaskDetailModal taskId={taskId} open={taskId !== null} onOpenChange={(open) => !open && setTaskId(null)} onChanged={() => void query.refetch()} workspaceMembers={selectedTask?.workspaceMembers ?? []} availableTasks={(query.data ?? []).filter((task) => task.sprintId === selectedTask?.sprintId)} />
    </>
  );
}
